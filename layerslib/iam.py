...
def policies_for_group(group_name):
    return [
        _M(client().get_group_policy(GroupName=group_name , PolicyName=p))
        for p in group_policies_for_group(group_name)
    ]
...
def group_policies_for_user(user_name, groups=[]):
    groups = groups or groups_for_user(user_name)
    return list(itertools.chain(*[
        policies_for_group(g['GroupName'])
        for g in groups
    ]))


def policies_for_user(user_name, groups=[]):
    groups = groups or groups_for_user(user_name)
    return dict(
        Groups=groups,

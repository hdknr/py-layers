...
def findRecipients(message_obj):
    addrs = getaddresses(
        chain(message_obj.get_all('to', []), message_obj.get_all('cc', []), 
...
def sendMessage(sender, forwarded, message_text, region_name, original):
    try:
        ...
        SES.send_raw_email(
            Source=sender,
            Destinations=[forwarded], RawMessage=dict(Data=message_text))
...
def forwardMessage(message_obj, settings, recipients=None):
    ...
    for recipient in recipients:
        ...
        for address in forwards:
            sendMessage(
                settings['sender'], address, msg_string, settings['region_name'], recipient)


def forwardMessageInBucket(messageId, s3bucket, s3prefix, recipients=[]):
